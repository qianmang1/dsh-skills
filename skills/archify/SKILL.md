...
用本技能画图时默认产出中英混合（文档式混排），便于中文读者直接看图。用户明确要全英文或全中文时除外。

- 名字区保持英文原文（能 grep 到代码/配置/文档的 token）：components/nodes/states/participants/steps 的 label、boundaries[].label、tag、id/from/to/variant/kind/type 等结构字段。
- 叙述区用中文（代码里搜不到的说明）：sublabel、cards[].title/items、views[].label/note、connections[].label、meta.title/subtitle。
- 交付前语言门禁：node scripts/check-zh.mjs <候选.json>。名字字段含中文即报错并列出位置；通过后再 validate + deliver。
- 本策略为本地附加改动：重新安装/升级技能包后需重新应用（复制 scripts/check-zh.mjs 进本目录，并在 SKILL.md 末尾重挂本节）。
