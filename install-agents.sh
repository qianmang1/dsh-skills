...
# 用法：curl -fsSL https://raw.githubusercontent.com/qianmang1/dsh-skills/main/install-agents.sh | sh
#       或本地执行：sh install-agents.sh [技能名]   # 默认 dsh-hermes-plugin
# AGENTS_HOME 未设置时回退到 ~/.agents（与 .agents 生态技能同位）
# 安全顺序：先下载解压到临时暂存区，校验成功后才替换目标（失败不影响已有安装）
set -e

SKILL="${1:-dsh-hermes-plugin}"
AGENTS_HOME="${AGENTS_HOME:-$HOME/.agents}"
DEST="$AGENTS_HOME/skills/$SKILL"
