# dsh-skills 一键安装脚本（user-agents，rank 500：<agentsHome>/skills）
# 用法：irm https://raw.githubusercontent.com/qianmang1/dsh-skills/main/install-agents.ps1 | iex
#       或本地执行：.\install-agents.ps1 [-Skill dsh-hermes-plugin]
# AGENTS_HOME 未设置时回退到 %USERPROFILE%\.agents（与 .agents 生态技能同位）
...
$agentsHome = $env:AGENTS_HOME
if (-not $agentsHome) { $agentsHome = Join-Path $env:USERPROFILE ".agents" }
$dest = Join-Path $agentsHome "skills\$Skill"
...
Get-ChildItem -Name (Join-Path $agentsHome "skills")
